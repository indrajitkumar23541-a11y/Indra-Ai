...
2. Create a project directory for Indra Ai
...
1. Download [`.env.template`][.env.template] and save it as `.env` in the Indra Ai folder.
...
!!! note "Docker only supports headless browsing"
    Indra Ai uses a browser in headless mode by default: `HEADLESS_BROWSER=True`.
    Please do not change this setting in combination with Docker, or Indra Ai will crash.
...
## Running Indra Ai with Docker

After following setup instructions above, you can run Indra Ai with the following command:

```shell
docker compose run --rm auto-gpt
```

This creates and starts an Indra Ai container, and removes it after the application stops.
...
* Run Indra Ai:
    ...
    ```
* Run Indra Ai in TTY mode, with continuous mode.
    ```shell
    docker compose run --rm auto-gpt run --continuous
    ```
* Run Indra Ai in TTY mode and install dependencies for all active plugins:
    ```shell
